...
- **Prevent the attribute table from overwriting the layer display expression with `COALESCE`** — in upstream QGIS, when the attribute table is opened, `QgsDualView::columnBoxInit()` recognizes that the current display expression is a simple field and automatically triggers `QgsDualView::previewColumnChanged()`. That function wraps the field in `COALESCE( "field", '<NULL>' )` so the feature list shows `<NULL>` for null values, but it also persists the wrapped expression on the layer via the `displayExpressionChanged` signal (connected to `QgsDualView::previewExpressionChanged()` → `mLayer->setDisplayExpression()`). This silently replaces the original column with a complex expression, which in turn breaks `QgsVectorLayer::displayField()` (it only recognizes a simple field reference and returns an empty string otherwise). The fix in `previewColumnChanged()` blocks `displayExpressionChanged` while setting the `COALESCE` form on the feature list view (so the `<NULL>` display behavior is preserved) and explicitly persists the plain field name on the layer instead of the wrapped expression. This also self-heals layers already affected, since `columnBoxInit()` still matches the existing `COALESCE(...)` form and the corrected trigger restores the simple field.

- **Client-side evaluation of WFS `QgsFeatureRequest` filter expressions** — `QgsWFSSharedData::computedExpression()` no longer pushes filter expressions to the WFS server. It returns an empty string so that no server-side `<Filter>` is generated for feature request expressions; the filtering is still performed client-side by `QgsAbstractFeatureIterator::nextFeatureFilterExpression()`. This is a temporary workaround for servers (e.g. GeoServer) that misbehave when receiving the generated OGC filter, while keeping the result set correct by falling back to local evaluation.
